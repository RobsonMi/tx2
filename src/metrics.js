...
module.exports = {
  ...
  prepareData: function () {
    ...
    Object.keys(this._metrics).forEach((probe_name) => {
      if (typeof this._metrics[probe_name].value == "undefined") return false;

      cooked_data[probe_name] = {
        value: getValue(this._metrics[probe_name].value),
      };

      if (this._metrics[probe_name].unit) cooked_data[probe_name].unit = this._metrics[probe_name].unit;
  ...
  metric: function (opts, unit, val) {
    ...
    if (!name) return console.error("[PX2][Metric] Name not defined");
  ...
  histogram: function (opts) {
    if (!opts.name) return console.error("[Metric][Histogram] Name not defined");
   ...
   */
  meter: function (opts) {
    if (!opts.name) return console.error("[Metric][Meter] Name not defined");
  ...
  counter: function (opts) {
    let name,
      unit,
      agg_type = DEFAULT_AGGREGATION;

    if (typeof opts == "string") name = opts;
    else {
      name = opts.name;
      unit = opts.unit;
    }

    if (!name) return console.error("[Metric][Counter] Name not defined");
